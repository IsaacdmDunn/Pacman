...
void Pacman::LoadContent()
{
	...
	//Load sounds
	_pop->Load("Sounds/waka.wav");
	_deathSound->Load("Sounds/deathSound.wav");
	_titleMusic->Load("Sounds/titleMusic.wav");
	_gameOverMusic->Load("Sounds/gameOver.wav");
	_levelWin->Load("Sounds/levelWin.wav");

	Audio::Play(_titleMusic); //plays game music

	_pacman->ghostScared = false; //stops ghosts moving in main menu
...
void Pacman::LoadTileMap()
{
	...
	//resets level back to 1 when levelNum equals 3
	else if (_level->levelNum == 3)
	{
		level = "levels/level_layout1.txt";
		_level->levelNum = 1;
	}
...
void Pacman::CheckLevelCompletion() 
{
	if (_level->levelDotCount == 0) {
		...
		_level->levelNum++; //change level 
		LoadTileMap(); //load new map
	}
}
